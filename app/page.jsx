...
import {
  supabase, sendOTP, verifyOTP, signOut,
...
function LoginScreen() {
  ...
  const handleVerify = async () => {
    ...
    if (error) { setMsg('코드가 올바르지 않아요. 다시 확인해주세요.'); setLoading(false); return }
    setLoading(false)
  }

  const inp = {
    background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)',
    borderRadius:14, padding:'0 16px', fontSize:15, fontFamily:'inherit',
    outline:'none', color:'#fff', width:'100%', maxWidth:300, height:50,
  }
    ...
    <div style={{ minHeight:'100vh', background:navy, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:"'Noto Sans KR',sans-serif" }}>
      ...
      {step === 'email' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10, width:'100%' }}>
          ...
          <Btn onClick={handleSendOTP}
            style={{ ...inp, background: email.trim() ? purple : 'rgba(255,255,255,0.15)',
              fontWeight:700, cursor: email.trim() ? 'pointer' : 'default', textAlign:'center',
              display:'flex', alignItems:'center', justifyContent:'center' }}>
            {loading ? '전송 중...' : '인증 코드 받기'}
          </Btn>
          <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:8, textAlign:'center', lineHeight:1.7 }}>
            이메일로 6자리 코드를 보내드려요<br />회원가입 없이 바로 로그인돼요
          </div>
        </div>
      )}
      ...
      {step === 'otp' && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10, width:'100%' }}>
          <div style={{ fontSize:13, color:'#9FE1CB', marginBottom:4, textAlign:'center' }}>
            📧 <strong>{email}</strong>로<br />6자리 코드를 보냈어요
          </div>
          <input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g,'').slice(0,6))}
            onKeyDown={e => e.key === 'Enter' && handleVerify()}
            placeholder="123456" type="tel" maxLength={6}
            style={{ ...inp, textAlign:'center', fontSize:24, letterSpacing:10, fontWeight:700 }} />
          <Btn onClick={handleVerify}
            style={{ ...inp, background: otp.length === 6 ? purple : 'rgba(255,255,255,0.15)',
              fontWeight:700, cursor: otp.length === 6 ? 'pointer' : 'default',
              display:'flex', alignItems:'center', justifyContent:'center' }}>
            {loading ? '확인 중...' : '로그인'}
          </Btn>
          <Btn onClick={() => { setStep('email'); setOtp(''); setMsg('') }}
            style={{ background:'none', color:'rgba(255,255,255,0.4)', fontSize:12, marginTop:4 }}>
            ← 이메일 다시 입력
          </Btn>
        </div>
      )}

      {msg && <div style={{ marginTop:14, fontSize:12, color:'#F0997B', textAlign:'center', maxWidth:280 }}>{msg}</div>}
    </div>
  )
}
...
function OikosApp({ session }) {
  ...
  const openKakao = (oikos, msg = '') => {
    const encoded = encodeURIComponent(msg || `안녕하세요 ${oikos.name}님!`)
...
조건: "하늘에 계신 아버지 하나님"으로 시작, "예수님의 이름으로 기도합니다 아멘"으로 마무리, 이름 친근하게 부르기, 200~300자, 기도문만 출력`
      ...
      setPState('result')
    } catch { setPResult('오류가 발생했습니다.'); setPState('result') }
  }
            ...
            return (
              <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                ...
                <div style={{ width:26, height:26, borderRadius:'50%',
                  ...
                  border: isToday && !done ? '2px solid #9FE1CB' : 'none',
      ...
      {todayOikos ? (
        <>
            ...
            <div style={{ background:purple, borderRadius:16, padding:14, display:'flex', gap:12, alignItems:'center' }}>
              ...
              <Btn onClick={() => handlePrayed(todayOikos.id)}
                style={{ background: prayedToday(prayerLogs, todayOikos.id) ? '#5DCAA5' : '#9FE1CB',
                  borderRadius:20, padding:'7px 11px', fontSize:11, fontWeight:700, color:'#085041', whiteSpace:'nowrap' }}>
          ...
          <div style={{ padding:'14px 16px 0' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {[
                ...
                { icon:'💬', label:'카카오 안부', desc:'원클릭 메시지',     bg:'#FEE500', ic:navy,      fn:() => openKakao(todayOikos, getDayAction(todayOikos.day_in_challenge||1).kakaoMsg) },
                { icon:'☕', label:'기프티콘',   desc:'따뜻한 선물',        bg:'#E1F5EE', ic:'#0F6E56', fn:() => openGift(todayOikos) },
                { icon:'✉️', label:'초청장 전송', desc:'전도축제 D-18',    bg:'#FAECE7', ic:'#993C1D', fn:() => openKakao(todayOikos, '저희 교회 전도축제에 함께하실 수 있으세요? 🙏') },
              ].map((a,i) => (
          ...
          <div style={{ margin:'14px 16px 0' }}>
            ...
            {(() => { const da = getDayAction(todayOikos.day_in_challenge||1); return (
              <div style={{ background:'#fff', border:`1.5px solid ${SM[todayOikos.stage]?.bar||'#7F77DD'}`, borderRadius:14, padding:14, display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:'#888780' }}>{todayOikos.day_in_challenge||1}일차 · 지금이 딱 좋은 타이밍이에요</div>
                </div>
                <Btn onClick={() => openKakao(todayOikos, da.kakaoMsg)} style={{ background:purple, borderRadius:10, padding:'7px 12px', fontSize:11, fontWeight:700, color:'#fff', whiteSpace:'nowrap' }}>실행</Btn>
              </div>
            )})()}
          </div>
        </>
        ...
        <div style={{ padding:'40px 20px', textAlign:'center' }}>
          ...
          <div style={{ fontSize:13, color:'#888780', lineHeight:1.6, marginBottom:20 }}>아래 + 버튼을 눌러<br />첫 번째 오이코스를 추가해요</div>
        </div>
      )}
        ...
        {addStep===1 ? (
          <>
            ...
            <Btn onClick={()=>form.name.trim()&&setAddStep(2)}
              style={{ width:'100%', height:48, background:form.name.trim()?navy:'#d3d1c7', borderRadius:14, color:'#fff', fontSize:15, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              다음 단계 →
            </Btn>
          </>
        ) : (
      ...
      {pState==='select' && (
        <div style={{ flex:1, overflowY:'auto' }}>
          <div style={{ display:'flex', gap:12, padding:'14px 18px', overflowX:'auto' }}>
            {oikosList.map((o,i) => {
              const [bg,cl]=AVC[i%AVC.length]; const s=selId===o.id
              return (
      ...
      {pState==='result' && (
        <div style={{ flex:1, overflowY:'auto', padding:'14px 18px' }}>
          ...
          <div style={{ background:'#fff', border:'0.5px solid #d3d1c7', borderRadius:16, padding:'13px 14px', marginBottom:20 }}>
            ...
            {selOikos && [
              { icon:'💬', bg:'#FEE500', name:'카카오 안부 메시지', sub:'메시지 전송', fn:()=>openKakao(selOikos, getDayAction(selOikos.day_in_challenge||1).kakaoMsg) },
              { icon:'☕', bg:'#E1F5EE', name:'기프티콘 보내기', sub:'카카오 선물하기 연결', fn:()=>openGift(selOikos) },
